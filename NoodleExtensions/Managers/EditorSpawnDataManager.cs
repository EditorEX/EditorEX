    ...
    internal class EditorSpawnDataManager
    {
        ...
        private float GetGravityBase(float noteLineLayer, float beforeJumpLineLayer)
        {
            return HighestJumpPosYForLineLayer(noteLineLayer)
                - LineYPosForLineLayer(beforeJumpLineLayer);
        }
        ...
        private float HighestJumpPosYForLineLayer(float lineLayer)
        {
            ...
            return (0.525f * lineLayer)
                + 0.858333f
                + _movementData._jumpOffsetYProvider.jumpOffsetY;
        }
        ...
        )
        {
            ...
            obstacleOffset.x +=
                (width - StaticBeatmapObjectSpawnMovementData.kNoteLinesDistance) * 0.5f;
            result = new ObstacleSpawnData(obstacleOffset, width, obstacleHeight);
        ...
        internal bool GetJumpingNoteSpawnData(NoteEditorData? noteData, out NoteSpawnData? result)
        {
            ...
            result = new NoteSpawnData(noteOffset2, noteOffset2, noteOffset, gravity);

            return true;
        }

        internal bool GetSliderSpawnData(
            BaseSliderEditorData? sliderData,
            out SliderSpawnData? result
        )
        {
            if (
                sliderData == null
                || !(
                    _editorDeserializedData?.Resolve(
                        sliderData,
                        out EditorNoodleSliderData? noodleData
                    ) ?? false
                )
                || noodleData == null
            )
            ...
            Vector3 headOffset = GetNoteOffset(
                headLineIndex,
                gravityOverride ? headLineLayer : headStartLineLayer
            );
            Vector3 tailOffset = GetNoteOffset(
                tailLineIndex,
                gravityOverride ? tailLineLayer : tailStartLineLayer
            );
            float headGravity = GetGravityBase(
                headLineLayer,
                gravityOverride ? headLineLayer : headStartLineLayer
            );
            float tailGravity = GetGravityBase(
                tailLineLayer,
                gravityOverride ? tailLineLayer : tailStartLineLayer
            );

            result = new SliderSpawnData(headOffset, headGravity, tailOffset, tailGravity);
