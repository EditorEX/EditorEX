    ...
    internal class CameraLock : IAffinity
    {
        ...
        private bool CameraControllerUpdate(BeatmapEditor360CameraController __instance)
        {
            ...
            if (!_activeViewMode.Mode.LockCamera)
            {
                ...
                var movementFlags = __instance._movementDirection;
                if (movementFlags.HasFlag(BeatmapEditor360CameraController.Direction.Forward))
                {
                    vector2 = __instance._uiCameraTransform.forward;
                }
                if (movementFlags.HasFlag(BeatmapEditor360CameraController.Direction.Backward))
                {
                    vector2 = -__instance._uiCameraTransform.forward;
                }
                ...
                if (movementFlags.HasFlag(BeatmapEditor360CameraController.Direction.Right))
                {
                    vector3 = __instance._uiCameraTransform.right;
                }
                if (movementFlags.HasFlag(BeatmapEditor360CameraController.Direction.Left))
                {
                    vector3 = -__instance._uiCameraTransform.right;
                }
                ...
                if (movementFlags.HasFlag(BeatmapEditor360CameraController.Direction.Up))
                {
                    vector4 = __instance.transform.up;
                }
                if (movementFlags.HasFlag(BeatmapEditor360CameraController.Direction.Down))
                {
                    vector4 = -__instance.transform.up;
                }
