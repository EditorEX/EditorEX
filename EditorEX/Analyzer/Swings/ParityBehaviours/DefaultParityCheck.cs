...
namespace EditorEX.Analyzer.Swings
{
    ...
    public class DefaultParityCheck : IParityMethod
    {
        ...
        private Dictionary<int, Func<Vector2, int, int, Parity, bool>> _bombDetectionConditions =
            new()
            {
                {
                    0,
            ...
            for (int i = 0; i < bombs.Count; i++)
            {
                ...
                note = lastCut
                    .notesInCut.Where(note =>
                        note.column == lastCut.endPositioning.x
                        && note.row == lastCut.endPositioning.y
                    )
                    .FirstOrDefault();
        ...
        )
        {
            ...
            if (lastCut.endPositioning.angle == 180)
            {
                ...
                if (altNextAFN >= 0)
                {
                    return (lastCut.sliceParity == Parity.Forehand)
                        ? Parity.Backhand
                        : Parity.Forehand;
                }
                else
