        ...
        public void Load(string projectPath)
        {
            ...
            ValueTuple<string, string> correctedPathAndFilename =
                BeatmapProjectFileHelper.GetCorrectedPathAndFilename(
                    projectPath,
                    customBeatmapLevelSaveData.audio.songFilename
                );
            ...
            ValueTuple<string, string> correctedPathAndFilename2 =
                BeatmapProjectFileHelper.GetCorrectedPathAndFilename(
                    projectPath,
                    customBeatmapLevelSaveData.coverImageFilename
                );
                ...
                .colorSchemes.Select(
                    (SerializedCustomBeatmapLevelSaveData.ColorScheme colorScheme) =>
                        new BeatmapLevelColorSchemeEditorData(
                            ...
                            GetColorFromHtmlString(colorScheme.environmentColor1Boost)
                        )
                )
                .ToList();

            string defaultEnvironment = _environmentsListModel
                .GetLastEnvironmentInfoWithType(EnvironmentType.Normal)!
                .serializedName;
            List<EnvironmentName> environmentNames = customBeatmapLevelSaveData
                .environmentNames.Select(
                    (string environmentName) => new EnvironmentName(environmentName)
                )
                .ToList<EnvironmentName>();
            ...
            foreach (
                SerializedCustomBeatmapLevelSaveData.DifficultyBeatmap difficultyBeatmap in customBeatmapLevelSaveData.difficultyBeatmaps
            )
            {
                BeatmapCharacteristicSO beatmapCharacteristicBySerializedName =
                    _beatmapCharacteristicCollection.GetBeatmapCharacteristicBySerializedName(
                        difficultyBeatmap.characteristic
                    );

                BeatmapDifficulty beatmapDifficulty2;
                BeatmapDifficulty beatmapDifficulty = (
                    difficultyBeatmap.difficulty.BeatmapDifficultyFromSerializedName(
                        out beatmapDifficulty2
                    )
                        ? beatmapDifficulty2
                        : BeatmapDifficulty.Easy
                );

                DifficultyBeatmapData difficultyBeatmapData = new DifficultyBeatmapData(
                    beatmapCharacteristicBySerializedName,
                    beatmapDifficulty,
                    difficultyBeatmap.beatmapDataFilename,
                    difficultyBeatmap.lightshowDataFilename,
                    (
                        difficultyBeatmap.environmentNameIdx >= 0
                        && difficultyBeatmap.environmentNameIdx < environmentNames.Count
                    )
                        ? environmentNames[difficultyBeatmap.environmentNameIdx]
                        : defaultEnvironment
                );

                difficultyBeatmapData.noteJumpMovementSpeed =
                    difficultyBeatmap.noteJumpMovementSpeed;
                difficultyBeatmapData.noteJumpStartBeatOffset =
                    difficultyBeatmap.noteJumpStartBeatOffset;
                difficultyBeatmapData.mappers = difficultyBeatmap.beatmapAuthors.mappers.ToArray();
                difficultyBeatmapData.lighters =
                    difficultyBeatmap.beatmapAuthors.lighters.ToArray();
                difficultyBeatmapData.colorScheme = (
                    (
                        difficultyBeatmap.beatmapColorSchemeIdx >= 0
                        && difficultyBeatmap.beatmapColorSchemeIdx < colorSchemes.Count
                    )
                        ? colorSchemes[difficultyBeatmap.beatmapColorSchemeIdx]
                        : null
                );
            ...
            Dictionary<string, CustomData> beatmapCustomDatasByFilename =
                customBeatmapLevelSaveData.difficultyBeatmaps.ToDictionary(
                    x => x.beatmapDataFilename,
                    x => x.customData
                );
            ...
            beatmapLevelDataModel.UpdateWith(
                ...
                coverFileName,
                coverPath,
                new EnvironmentName?(defaultEnvironment),
                colorSchemes,
                beatmapDatas,
                true
            );
