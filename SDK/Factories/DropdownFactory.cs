...
namespace EditorEX.SDK.Factories
{
    public class DropdownFactory
    {
        ...
        public SimpleTextEditorDropdownView Create(
            Transform parent,
            string text,
            bool accent,
            float inputWidth,
            IEnumerable<string> options,
            Action<DropdownEditorView, int> onClick
        )
        {
            var dropdown = _container
                .InstantiatePrefab(_prefabCollector.GetTextDropdownPrefab().transform.parent)
                .GetComponentInChildren<SimpleTextEditorDropdownView>();
            ...
            var rootSizeFitter =
                dropdown.transform.parent.gameObject.AddComponent<ContentSizeFitter>();
            ...
            var rootHorizontal =
                dropdown.transform.parent.gameObject.AddComponent<HorizontalLayoutGroup>();
            ...
            var textObj = dropdown
                .transform.parent.Find("BeatmapEditorLabel")
            ...
            if (accent)
            {
                ...
                var transition = _transitionCollector.GetTransition<ColorTransitionSO>(
                    "EditorDropdown/CellBackground"
                );

                dropdown
                    ._tableView.transform.Find("Background4px")
                    .Find("Background")
                    .GetComponent<ImageView>()
                    ._colorSo = color;

                dropdown
                    ._text.transform.parent.GetComponentInChildren<ColorGraphicStateTransition>()
                    ._transition = transition;

                dropdown._cellPrefab = GameObject.Instantiate(dropdown._cellPrefab);
                dropdown
                    ._cellPrefab.GetComponentInChildren<ColorGraphicStateTransition>()
                    ._transition = transition;
